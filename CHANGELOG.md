...
- Added the frontend test suites: the `api.ts` command-name/argument mapping the backend expects, App-level handling of backend events (`show-settings`, `contexts-changed`), the `Ctrl+,`/`Cmd+,` settings shortcut, blur-collapse/focus-restore sizing, two-tier sidebar rendering with context-menu shortcut/rename/delete flows, Settings save/merge/fallback/error handling, and DetailPanel drop handling (move vs Shift-copy, removal, rejected-drop snap-back) driven through the events `svelte-dnd-action` emits. ([#78](https://github.com/conveen/context-manager/pull/78))
- Added cross-layer contract fixtures (`fixtures/`): the backend→frontend event names and a representative `AppData` JSON, asserted by both test suites so an IPC-boundary rename or shape change on either side fails a test; plus the manual pre-release checklist (`docs/MANUAL_TESTING.md`) covering the OS-dependent behaviors automated tests deliberately exclude. ([#79](https://github.com/conveen/context-manager/pull/79))
