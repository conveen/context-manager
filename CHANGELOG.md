...
- Global Context shortcuts no longer all die when one of them collides. Each accelerator is now registered on its own, so a combination another application already owns costs only itself instead of every shortcut registered after it — previously a collision on `Ctrl+Alt+0`, the first one registered, left the whole feature silently dead on Windows. Shortcuts that can't be registered are named in an error toast rather than only in a release build's discarded stderr. On Windows the default modifier is now `Ctrl+Alt+Win`, which stays clear of the AltGr (`Ctrl+Alt`) combinations that resident software commonly claims; existing installs keep the modifier they have, and it can be changed in Settings. ([#97](https://github.com/conveen/context-manager/pull/97))
