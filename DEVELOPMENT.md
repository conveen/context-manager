...
The build container, defined in [build-support/docker/Dockerfile](build-support/docker/Dockerfile), is based on [Ubuntu 22.04 LTS (jammy)](https://hub.docker.com/_/ubuntu).
It includes a `minimal` installation of [Rustup and Rust](https://www.rust-lang.org/tools/install),
[llvm-cov](https://github.com/taiki-e/cargo-llvm-cov) for coverage report generation,
[asdf](https://asdf-vm.com/) for tool version management,
and [Node.js](https://nodejs.org/) (via asdf) for the Tauri frontend build.
...
Initialize the project with a package name and target type (bin or lib):
...
The default command, `build`, checks the format and types, lints the code, runs unit tests, and compiles the package(s):
...
|-------------  |------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| build         | `./run.sh build`             | Build the application. Before building, this command checks the code for errors, checks formatting, lints, and runs test coverage, then compiles the crate and builds the Tauri application bundle. This is the default command. |
| build-base    | `./run.sh build-base`        | Build the build container image.                                                                                                                                                                             |
| bundle        | `./run.sh bundle`            | Build the [Tauri](https://tauri.app/) application bundle with `tauri build`.                                                                                                                                 |
| check         | `./run.sh check`             | Check the Rust package for errors with [cargo-check](https://doc.rust-lang.org/cargo/commands/cargo-check.html) and type-check the frontend with [svelte-check](https://github.com/sveltejs/language-tools).  |
| check-deps    | `./run.sh check-deps`        | Check dependencies for license compliance, security notices, and trusted sources with [cargo-deny](https://docs.rs/cargo-deny/latest/cargo_deny/). Note: cargo-deny is currently disabled in the build container; install it locally to use this command. |
...
| dev           | `./run.sh dev`               | Start the [Tauri](https://tauri.app/) development server (frontend + backend) with `tauri dev`.                                                                                                              |
| exec          | `./run.sh exec COMMAND`      | Execute arbitrary shell command (`COMMAND`).                                                                                                                                                                 |
| fmt           | `./run.sh fmt`               | Format Rust code with [Rustfmt](https://github.com/rust-lang/rustfmt) and frontend code with [Biome](https://biomejs.dev/).                                                                                  |
| fmt-check     | `./run.sh fmt-check`         | Check Rust formatting with [Rustfmt](https://github.com/rust-lang/rustfmt) and frontend formatting with [Biome](https://biomejs.dev/) without modifying any files.                                            |
| init          | `./run.sh init`              | Initialize repository with a package name and target type (should only be run once - see [Initializing the Repo](#initializing-the-repo)).                                                                   |
| lint          | `./run.sh lint`              | Lint Rust code with [Clippy](https://github.com/rust-lang/rust-clippy) and frontend code with [Biome](https://biomejs.dev/).                                                                                 |
| make-docs     | `./run.sh make-docs`         | Compile package documentation with [cargo-doc](https://doc.rust-lang.org/cargo/commands/cargo-doc.html).                                                                                                     |
...
| shell         | `./run.sh shell`             | Start a Bash shell to run interactive commands.                                                                                                                                                              |
